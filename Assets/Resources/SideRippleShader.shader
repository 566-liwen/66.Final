...
;
            ...
            v2f vert (appdata v)
            {
                ...
                o.vertex = v.vertex;
               // o.vertex.z += sin(v.vertex.x * _Frequency + _Time.y * _AnimationSpeed) * randAmp;
                float3 distval = o.vertex;

                o.tangent = v.tangent;
                float3 posPlusTangent = v.vertex + v.tangent * 0.01;
               // posPlusTangent.z += sin(posPlusTangent.x * _Frequency + _Time.y * _AnimationSpeed) * randAmp;

                float3 bitangent = cross(v.normal, v.tangent);
                o.bitangent = float4(bitangent, 1);
                float3 posPlusBitangent = v.vertex + bitangent * 0.01;
                //posPlusBitangent.z += sin(posPlusBitangent.x * _Frequency + _Time.y * _AnimationSpeed) * randAmp;
