Shader "Unlit/RippleShader"
{
    Properties
    {
        //_RippleRate ("Ripple Rate", Range(0, 1.5)) = 0.7
        //_RippleFrequency ("Ripple Frequency", Range(0, 30)) = 0.1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 modifyVertexCircle(float3 o_vertex, float2 o_uv, float timeY) {
                // ref: https://www.youtube.com/watch?v=kfM-yu0iQBk
                if (o_uv.x == 1 || o_uv.y == 1) {
                    return o_vertex;
                }
                float2 uvsCentered = o_uv * 2 - 1;
                float distance = length (uvsCentered);
                if (distance > 0.99) {
                    return o_vertex;
                }
                float wave = cos ((distance * _RippleFrequency - timeY * _RippleAnimationSpeed)  * 30 )* 0.5 + 0.5;
                wave *= 1 - distance;
                return float3(o_vertex.x, wave * _RippleRate, o_vertex.z);
            }
            ...
            v2f vert (appdata v)
            {
                ...
                float randAmp = (_SinTime.w /50 + 0.05f) * _RippleRate;
                randAmp = _RippleRate;
                float timeY = _Time.y;

                o.vertex = v.vertex;
                o.vertex = float4(modifyVertexCircle(o.vertex.xyz, v.uv, timeY), v.vertex.w);
                ...
                //posPlusTangent = modifyVertexCircle(posPlusTangent, v.uv, timeY);
                posPlusTangent = modifyVertexCircle(posPlusTangent, v.uv, timeY);
               ...
               // posPlusBitangent.y += sin(posPlusBitangent.x * _Frequency + _Time.y * _AnimationSpeed) * randAmp;
                posPlusBitangent = modifyVertexCircle(posPlusBitangent, v.uv, timeY);
